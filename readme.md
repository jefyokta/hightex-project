# Hightex Project 
buat readme untuk menjelaskan kalo ada masalah di aplikasi submit issue:



Berikut versi README yang copyable, menggunakan link Markdown standar:

## 📝 Melaporkan Masalah (Issue)

Jika Anda menemukan bug, mengalami error, atau memiliki saran pengembangan, silakan laporkan melalui GitHub Issues.

## 🚀 Cara Melaporkan Masalah

### Buka halaman Issues pada repository ini:
👉 [issue](https://github.com/jefyokta/hightex-project/issues)

### Klik "New Issue".

### Jelaskan masalah dengan jelas:

- Langkah yang menyebabkan error

- Perilaku yang diharapkan

- Perilaku yang terjadi

- Screenshot atau log (jika ada)

### 💡 Kenapa Perlu Submit Issue?

Pelaporan masalah membantu kami memperbaiki aplikasi lebih cepat dan membuatnya lebih stabil serta sesuai kebutuhan pengguna.
Masukan Anda sangat berarti!



# not ai generation
interested to contibute to the project?
## contact me

- ig: jefyokta_
- email : jefyokta50@gmail.com / jefyokta50@icloud.com

---

## Download HighTex Desktop App

Silakan unduh installer aplikasi sesuai dengan sistem operasi yang Anda gunakan melalui tautan unduhan langsung di bawah ini:

| Sistem Operasi | Format | Tautan Unduhan Langsung |
| :--- | :--- | :--- |
| **Windows** | .exe | [Download untuk Windows](https://github.com/jefyokta/hightex-desktop/releases/download/v0.0.2/HighTex-Windows-0.0.2-Setup.exe) |
| **macOS** | .dmg | [Download untuk macOS](https://github.com/jefyokta/hightex-desktop/releases/download/v0.0.2/HighTex-Mac-0.0.2-Installer.dmg) |
| **Linux** | .AppImage | [Download untuk Linux](https://github.com/jefyokta/hightex-desktop/releases/download/v0.0.2/HighTex-Linux-0.0.2.AppImage) |
